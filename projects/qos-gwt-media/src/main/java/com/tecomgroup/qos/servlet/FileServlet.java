...
public class FileServlet implements HttpRequestHandler {
	...
	private void processRequest(HttpServletRequest request,
		...
		try {
			...
			FTPFile[] files = ftpClient.listFiles(probeConfigPath);
			for (FTPFile f : files) {
				if(f.isFile() && f.getName().equals(URLDecoder.decode(requestedFile, "UTF-8"))) {
					file = f;
					// Prepare some variables. The ETag is an unique identifier of the file.
					fileName = file.getName();
					length = file.getSize();
					timestamp = file.getTimestamp().getTimeInMillis();
					inputStream = ftpClient.retrieveFileStream(probeConfigPath + "/" + URLDecoder.decode(requestedFile, "UTF-8"));
					break;
				}
			}
			...
			LOGGER.error("Unable to find file:" + URLDecoder.decode(requestedFile, "UTF-8"), e);
		} finally {
			try {
		...
		String eTag = fileName + "_" + length + "_" + timestamp;
		...
		if (ifNoneMatch == null && ifModifiedSince != -1
				&& ifModifiedSince + 1000 > timestamp) {
			response.setStatus(HttpServletResponse.SC_NOT_MODIFIED);
			response.setHeader("ETag", eTag); // Required in 304.
			response.setDateHeader("Expires", expires); // Postpone cache with 1
														// week.
		...
		if (ifUnmodifiedSince != -1 && ifUnmodifiedSince + 1000 <= timestamp) {
			response.sendError(HttpServletResponse.SC_PRECONDITION_FAILED);
			return;
		}
			...
			if (ifRange != null && !ifRange.equals(eTag)) {
				try {
					long ifRangeTime = request.getDateHeader("If-Range"); // Throws
																			...
																			// if
																			// invalid.
					if (ifRangeTime != -1 && ifRangeTime + 1000 < timestamp) {
						ranges.add(full);
					}
		...
		response.setDateHeader("Timestamp", timestamp);
		...
		try {
			...
			input = new RandomAccessFile(tempFile, "r");
